...
- [x] **Iframe Simulator** (`tools/iframe-simulator.{html,js}`) — teste l'embedding d'une URL : drapeaux sandbox + allow (Permissions Policy) en chips cliquables, presets (aucune / stricte / typique), `referrerpolicy`/`loading`/dimensions, prévisualisation live de l'iframe, génération du markup HTML, **détection heuristique de blocage** via `contentWindow.location.href` (about:blank/throws), **sonde HEAD/GET** pour lire `X-Frame-Options` et `frame-ancestors` (fallback no-cors), **console postMessage** parent ↔ iframe, exemples (example.com, google.com, youtube/embed, Keycloak). Copie / export / wiki / téléchargement HTML autonome.
- [x] **SwaggerGuard** (`tools/swagger-guard.{html,js}`) — charge JSON/YAML (js-yaml), liste les opérations et codes de réponse, valide une réponse API via Ajv + ajv-formats, résout les `$ref` OpenAPI 3 (`components/schemas`) et Swagger 2 (`definitions`). **Toutes les contraintes JSON Schema sont vérifiées nativement par Ajv** : `type`, `required`, `enum`, `format`, `minLength`/`maxLength`, `minimum`/`maximum`, `pattern`, `minItems`/`maxItems`, etc. — rien à coder de plus. Génère un **exemple de réponse attendue** depuis le schéma (avec bouton "Utiliser comme réponse"), et met en **évidence les champs additionnels** non déclarés dans le schéma. Copie / export / wiki / téléchargement HTML autonome. **Bouton "Charger un exemple"** : mini-API `GET /users/{id}` (OpenAPI 3, schéma `User` avec `$ref`) + réponse qui **respecte toutes les contraintes de taille / format / pattern / enum** (`countryCode: "FR"` dans maxLength:2, `firstName`/`lastName` dans maxLength:50, `email` valide, `role` dans l'enum, `createdAt` au format date-time). Seule violation volontaire conservée : le champ additionnel `nickname` non déclaré dans le schéma, pour démontrer la détection des champs additionnels.
